...
| TM-31 | **Request/response desync on a refused request** | Every refusal in the bridge returned *before* the request body was read, while the transport is HTTP/1.1 keep-alive. The unread body therefore stayed on the socket and its bytes would be parsed as the **next** request, so a caller could fail the `Host` / `Content-Type` / size checks, keep the connection, and have the smuggled bytes accepted on the retry — defeating the exact guards TM-29 accepts as the perimeter. Found because the first real CI run after the workflow repair exposed a client `EPIPE` (`tests/test_mcp_http_security.py::test_oversized_body_is_refused` was racing the server's close against a 1 MiB write and lost on CI while passing locally). **Status: closed 2026-09-21** — `_deny()` now ends the connection (`Connection: close`, `close_connection = True`) rather than draining, since draining an intentionally oversized body is the resource sink the 1 MiB cap exists to refuse. Guarded by `test_refusals_close_the_connection` (raw socket, 415 and 400 paths, asserts the header) and by the rewritten `test_oversized_body_is_refused` (announces the length, sends no body); both were checked to fail against the pre-fix handler, so the gate is not decoration. Severity: **P1** (a bypass of the TM-29 perimeter on the same connection that failed it, plus a flaky gate). |
